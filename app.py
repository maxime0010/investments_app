...
def get_latest_portfolio_date():
    conn = mysql.connector.connect(**db_config)
    cursor = conn.cursor()
    cursor.execute("SELECT MAX(date) FROM portfolio20241124")
...
def performance():
    ...
    # Fetch actual portfolio values and S&P 500 data from portfolio20241124 table
    cursor.execute("""
        SELECT p.date, SUM(p.total_value) AS total_portfolio_value, 
               (SELECT close 
                FROM prices sp 
